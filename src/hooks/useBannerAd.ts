...
export function useBannerAd(options: BannerAdOptions): BannerAdState & { ad: BannerAd } {
  const ad = useReleasingSharedObject<BannerAd>(() => {
    ...
    return created;
    // `adaptiveKind` belongs here for the same reason width/height do: it changes what the
    // native side requests, and two sizes can share a width and height while differing on it.
  }, [options.adUnitId, options.size.width, options.size.height, options.size.adaptiveKind]);
