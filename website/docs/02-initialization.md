...
Creating an ad before `initialize()` has been called is not an error: the load is queued until initialization completes. But if `initialize()` is never called at all, the ad stays `loading` forever, so creating one while it hasn't run yet logs a `__DEV__` warning.
