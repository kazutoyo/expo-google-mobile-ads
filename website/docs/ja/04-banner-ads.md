...
広告インスタンスの生成（＝ロード開始）と画面への表示は独立している。次の画面へ遷移する前でも、アプリ起動時でも、広告だけ先に作っておける。
...
`<BannerAdView>` はマウント時にネイティブ View をアタッチし、**アンマウント時はデタッチするだけ**で広告を破棄しない。画面から離れて戻ってきても、再ロードなしに同じ広告が出る。明示的に破棄するなら `ad.release()` を呼ぶ。

## hooks

2つの hook は、広告のライフタイムをどこまで持つかが違う。
...
広告のライフタイムが画面と一致するなら（プリロードしないなら）`useBannerAd` でいい。
...
プリロード済みの広告を画面で出すだけなら `useBannerAdState`（前節の例）。生成も release もしないので、`ad` のライフタイムは呼び出し側が持つ。

もっと細かい状態変化（インプレッション、クリック、収益イベントなど）は `ad.addListener(...)` で直接購読する。
...
GMA のバナー自動リフレッシュは **AdMob 管理画面（広告ユニットごとの設定）の機能で、SDK の API ではない**。このライブラリは一切関与しない。間隔を変えるなら AdMob の管理画面で設定する。
